...
13. [與 Tutorial 的差異](#13-與-tutorial-的差異)
14. [名詞解釋](#名詞解釋)
...
```bash
# 在 repo 根目錄執行
./gradlew test       # 執行全部 55 個測試（33 單元/切片 + 22 BDD 情境，應全綠）
...
測試對應各層，並以最快者優先執行——這就是實務上的 TDD 金字塔（共 55 個測試：33 單元/切片 + 22 BDD 情境）：
...
flowchart TB
    ...
    E["BDD 端對端情境測試<br/>Cucumber + Gherkin（22 情境）<br/>真實 HTTP，走完整路徑（見 §12）"]
    D --> A --> W --> E
...
| Cucumber BDD（Testcontainers／WireMock 版） | **Cucumber + 記憶體資料源**（見 §12） | 已實作 22 個端對端情境；以記憶體取代真實容器 |
